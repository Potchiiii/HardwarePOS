...
<script>
  ...
  // show loading modal when switching tabs (navigation links in the sidebar)
  (function () {
    // selects links inside this sidebar that have a non-empty href and no onclick (skip logout)
    const sidebar = document.querySelector('.h-screen');
    if (!sidebar || typeof swal !== 'function') return;

    sidebar.querySelectorAll('a[href]:not([onclick])').forEach(a => {
      a.addEventListener('click', function (e) {
        const href = this.getAttribute('href');
        if (!href || href === '#' || href.startsWith('javascript:')) return;
        e.preventDefault();
        // show modal without buttons, prevent closing
        swal({
          title: "Loading...",
          text: "Please wait",
          buttons: false,
        ...
        // short delay to let modal render; then navigate
        setTimeout(() => {
